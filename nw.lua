...
local function makePill(row, ctx, get, set)
    ...
    render(get())
    -- Re-apply colours when the theme changes (pill switches accent/track).
    if ctx and ctx._refresh then
        table.insert(ctx._refresh, function()
            pill.BackgroundColor3 = get() and PALETTE.accent or PALETTE.track
        end)
    end
...
local CONFIG_DIR = "NewReality/configs/"
...
function Window:watermark(opts)
    ...
    if show.logo ~= false and iconAsset(opts.icon or "logo") then
        local ic = Instance.new("ImageLabel")
        ic.BackgroundTransparency = 1
        ic.Size = UDim2.new(0, 22, 0, 22)
        ic.Image = iconAsset(opts.icon or "logo") or ""
...
function Interface.new(opts)
    ...
    if not hasLogo and iconAsset("diamond") ~= nil then
        hasLogo = true
        logoName = "diamond"
    end
    if hasLogo then
